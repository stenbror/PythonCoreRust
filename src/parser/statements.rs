...
pub trait Statements {
    fn parse_statements_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_simple_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_small_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_expr_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_ann_assign(&mut self, start_pos: &u32, left_node: Box<ASTNode>) -> Box<ASTNode>;
    fn parse_statements_del_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_pass_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_flow_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_break_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_continue_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_return_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_yield_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_raise_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_import_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_import_name(&mut self) -> Box<ASTNode>;
    fn parse_statements_import_from(&mut self) -> Box<ASTNode>;
    fn parse_statements_import_as_name(&mut self) -> Box<ASTNode>;
    fn parse_statements_dotted_as_name(&mut self) -> Box<ASTNode>;
    fn parse_statements_import_as_names(&mut self) -> Box<ASTNode>;
    fn parse_statements_dotted_as_names(&mut self) -> Box<ASTNode>;
    fn parse_statements_dotted_name(&mut self) -> Box<ASTNode>;
    fn parse_statements_global_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_nonlocal_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_assert_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_compound_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_async_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_if_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_elif_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_else_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_while_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_for_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_try_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_finally_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_with_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_with_item(&mut self) -> Box<ASTNode>;
    fn parse_statements_except_stmt(&mut self) -> Box<ASTNode>;
    fn parse_statements_except_clause(&mut self) -> Box<ASTNode>;
    fn parse_statements_suite(&mut self) -> Box<ASTNode>;
}
...
impl Statements for PythonCoreParser {
    ...
    fn parse_statements_simple_stmt(&mut self) -> Box<ASTNode> {
        let start_pos = &mut self.lexer.get_position();
        ...
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected NEWLINE after statements!", &mut self.lexer.get_position())
            }
        }
        let end_pos = &mut self.lexer.get_position();
        nodes_list.reverse();
    ...
    fn parse_statements_ann_assign(&mut self, start_pos: &u32, left_node: Box<ASTNode>) -> Box<ASTNode> {
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected ':' in annotated assignment statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_del_stmt(&mut self) -> Box<ASTNode> {
        ...
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'del' in del statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_pass_stmt(&mut self) -> Box<ASTNode> {
        ...
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'pass' in pass statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_flow_stmt(&mut self) -> Box<ASTNode> {
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'break', 'continue', 'return', 'raise' or 'yield' in flow statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_break_stmt(&mut self) -> Box<ASTNode> {
        ...
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'break' in break statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_continue_stmt(&mut self) -> Box<ASTNode> {
        ...
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'continue' in continue statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_return_stmt(&mut self) -> Box<ASTNode> {
        ...
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'return' in return statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_raise_stmt(&mut self) -> Box<ASTNode> {
        ...
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'raise' in raise statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_import_stmt(&mut self) -> Box<ASTNode> {
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'import' or 'from' in import statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_import_name(&mut self) -> Box<ASTNode> {
        ...
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'import' in import statement!", &mut self.lexer.get_position())
            }
        }
    }
        ...
        match &*self.lexer.get_symbol() {
            Token::PyFrom( .. ) => {
                ...
                match ( nodes_list.len(), &*self.lexer.get_symbol() ) {
                    ( 0, Token::PyImport( .. ) ) => {
                        panic!("Syntax Error at {} - Expected dot(s) or 'from part' in import statement!", &mut self.lexer.get_position())
                    },
                    ( _ , Token::PyImport( .. ) ) => {}
                    _ => {
                        ...
                        match &*self.lexer.get_symbol() {
                            ...
                            Token::PyLeftParen( .. ) => {
                                ...
                                match &*self.lexer.get_symbol() {
                                    ...
                                    _ => {
                                        panic!("Syntax Error at {} - Expected ')' in import statement!", &mut self.lexer.get_position())
                                    }
                                }
                            },
                            _ => {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'from' in import statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_import_as_name(&mut self) -> Box<ASTNode> {
        ...
        match &*self.lexer.get_symbol() {
                ...
                match &*self.lexer.get_symbol() {
                    Token::PyAs( .. ) => {
                        ...
                        match &*self.lexer.get_symbol() {
                            ...
                            _ => {
                                panic!("Syntax Error at {} - Expected Name literal after 'as' in import statement!", &mut self.lexer.get_position())
                            }
                        }
                    },
                    _ => {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected Name literal in import statement!", &mut self.lexer.get_position())
            }
        }
    }
                    ...
                    match &*self.lexer.get_symbol() {
                        Token::PyDot( .. ) => {
                            ...
                            match &*self.lexer.get_symbol() {
                                ...
                                _ => {
                                    panic!("Syntax Error at {} - Expected Name literal after '.' in import statement!", &mut self.lexer.get_position())
                                }
                            }
                            true
                        },
        ...
        match &*self.lexer.get_symbol() {
            Token::PyGlobal( .. ) => {
                ...
                match &*self.lexer.get_symbol() {
                    ...
                    _ => {
                        panic!("Syntax Error at {} - Expected Name literal in global statement!", &mut self.lexer.get_position())
                    }
                }
                while
                    match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'global' in global statement!", &mut self.lexer.get_position())
            }
        }
    }
        ...
        match &*self.lexer.get_symbol() {
            Token::PyNonLocal( .. ) => {
                ...
                match &*self.lexer.get_symbol() {
                    ...
                    _ => {
                        panic!("Syntax Error at {} - Expected Name literal in nonlocal statement!", &mut self.lexer.get_position())
                    }
                }
                while
                    match &*self.lexer.get_symbol() {
    ...
    fn parse_statements_assert_stmt(&mut self) -> Box<ASTNode> {
        ...
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'assert' in assert statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_compound_stmt(&mut self) -> Box<ASTNode> {
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'if', 'while', 'for', 'try', 'with', 'def', 'class', 'async' or '@' statement!", &mut self.lexer.get_position())
            }
        }
    }
        ...
        match &*self.lexer.get_symbol() {
            Token::PyAsync( .. ) => {
                ...
                match &*self.lexer.get_symbol() {
                    ...
                    _ => {
                        panic!("Syntax Error at {} - Expected 'def', 'with' or 'for' after 'async' statement!", &mut self.lexer.get_position())
                    }
                }
            },
            _ => {
                panic!("Syntax Error at {} - Expected 'async' in async statement!", &mut self.lexer.get_position())
            }
        }
    }
        ...
        match &*self.lexer.get_symbol() {
            Token::PyIf( .. ) => {
                ...
                match &*self.lexer.get_symbol() {
                    ...
                    _ => {
                        panic!("Syntax Error at {} - Expected ':' in if statement!", &mut self.lexer.get_position())
                    }
                }
            },
            _ => {
                panic!("Syntax Error at {} - Expected 'if' in if statement!", &mut self.lexer.get_position())
            }
        }
    }
        ...
        match &*self.lexer.get_symbol() {
            Token::PyElif( .. ) => {
                ...
                match &*self.lexer.get_symbol() {
                    ...
                    _ => {
                        panic!("Syntax Error at {} - Expected ':' in elif statement!", &mut self.lexer.get_position())
                    }
                }
            },
            _ => {
                panic!("Syntax Error at {} - Expected 'elif' in elif statement!", &mut self.lexer.get_position())
            }
        }
    }
        ...
        match &*self.lexer.get_symbol() {
            Token::PyElse( .. ) => {
                ...
                match &*self.lexer.get_symbol() {
                    ...
                    _ => {
                        panic!("Syntax Error at {} - Expected ':' in else statement!", &mut self.lexer.get_position())
                    }
                }
            },
            _ => {
                panic!("Syntax Error at {} - Expected 'else' in else statement!", &mut self.lexer.get_position())
            }
        }
    }
        ...
        match &*self.lexer.get_symbol() {
            Token::PyWhile( .. ) => {
                ...
                match &*self.lexer.get_symbol() {
                    ...
                    _ => {
                        panic!("Syntax Error at {} - Expected ':' in while statement!", &mut self.lexer.get_position())
                    }
                }
            },
            _ => {
                panic!("Syntax Error at {} - Expected 'while' in while statement!", &mut self.lexer.get_position())
            }
        }
    }
                ...
                match &*self.lexer.get_symbol() {
                    Token::PyIn( .. ) => {
                        ...
                        match &*self.lexer.get_symbol() {
                            ...
                            _ => {
                                panic!("Syntax Error at {} - Expected ':' in for statement!", &mut self.lexer.get_position())
                            }
                        }
                    },
                    _ => {
                        panic!("Syntax Error at {} - Expected 'in' in for statement!", &mut self.lexer.get_position())
                    }
                }
            },
            _ => {
                panic!("Syntax Error at {} - Expected 'for' in for statement!", &mut self.lexer.get_position())
            }
        }
    }
        ...
        match &*self.lexer.get_symbol() {
            Token::PyTry( .. ) => {
                ...
                match &*self.lexer.get_symbol() {
                    ...
                    _ => {
                        panic!("Syntax Error at {} - Expected ':' in try statement!", &mut self.lexer.get_position())
                    }
                }
            },
            _ => {
                panic!("Syntax Error at {} - Expected 'try' in try statement!", &mut self.lexer.get_position())
            }
        }
    }
            ...
            Token::PyFinally( .. ) => {
                ...
                match &*self.lexer.get_symbol() {
                    Token::PyColon( .. ) => {
                        ...
                        let right_node = self.parse_statements_suite();
                        let end_pos = &mut self.lexer.get_position();
                        Box::new( ASTNode::FinallyStmt(*start_pos, *end_pos, symbol1, symbol2, right_node) )
                    },
                    _ => {
                        panic!("Syntax Error at {} - Expected ':' in finally statement!", &mut self.lexer.get_position())
                    }
                }
            },
            _ => {
                panic!("Syntax Error at {} - Expected 'finally' in finally statement!", &mut self.lexer.get_position())
            }
        } 
    }
    ...
    fn parse_statements_with_stmt(&mut self) -> Box<ASTNode> {
        ...
        match &*self.lexer.get_symbol() {
                ...
                match &*self.lexer.get_symbol() {
                    Token::PyLeftParen( .. ) => {
                        ...
                        match &*self.lexer.get_symbol() {
                            ...
                            _ => {
                                panic!("Syntax Error at {} - Expected ')' in with statement!", &mut self.lexer.get_position())
                            }
                        }
                    },
                    _ => {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'with' in with statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_except_stmt(&mut self) -> Box<ASTNode> {
        ...
        match &*self.lexer.get_symbol() {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected ':' in except statement!", &mut self.lexer.get_position())
            }
        }
    }
    ...
    fn parse_statements_except_clause(&mut self) -> Box<ASTNode> {
        ...
        match &*self.lexer.get_symbol() {
                        ...
                        match &*self.lexer.get_symbol() {
                            Token::PyAs( .. ) => {
                                ...
                                match &*self.lexer.get_symbol() {
                                    ...
                                    _ => {
                                        panic!("Syntax Error at {} - Expected Name after 'as' in except statement!", &mut self.lexer.get_position())
                                    }
                                }
                            },
                            _ => {
            ...
            _ => {
                panic!("Syntax Error at {} - Expected 'except' in except statement!", &mut self.lexer.get_position())
            }
        }
    }
        ...
        match &*self.lexer.get_symbol() {
            Token::Newline( .. ) => {
                ...
                match &*self.lexer.get_symbol() {
                    Token::Indent( .. ) => {
                        ...
                        while 
                            ...
                            match &*self.lexer.get_symbol() {
                                ...
                                _ => {
                                    panic!("Syntax Error at {} - Expected <DEDENT> in block statement!", &mut self.lexer.get_position())
                                }
                            }
                            let end_pos = &mut self.lexer.get_position();
                    ...
                    _ => {
                        panic!("Syntax Error at {} - Expected <INDENT> in block statement!", &mut self.lexer.get_position())
                    }
                }
            },
            _ => {
