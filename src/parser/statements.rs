...
mod tests {
    ...
    #[test]
    fn statements_expression_plus_assign() {
        let lexer = Box::new(PythonCoreTokenizer::new("a += b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
            ...
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 9, nodes, separators, nwl) => {
                        ...
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::PlusAssignStmt( 0 , 6 , left, symbol, right) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 2 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*symbol {
                                    Token::PyPlusAssign(2, 4, _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*right {
                                    ASTNode::AtomName( 5, 6, _ ) => assert!(true),
    ...
    #[test]
    fn statements_expression_minus_assign() {
        let lexer = Box::new(PythonCoreTokenizer::new("a -= b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
            ...
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 9, nodes, separators, nwl) => {
                        ...
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::MinusAssignStmt( 0 , 6 , left, symbol, right) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 2 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*symbol {
                                    Token::PyMinusAssign(2, 4, _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*right {
                                    ASTNode::AtomName( 5, 6, _ ) => assert!(true),
    ...
    #[test]
    fn statements_expression_mul_assign() {
        let lexer = Box::new(PythonCoreTokenizer::new("a *= b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
            ...
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 9, nodes, separators, nwl) => {
                        ...
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::MulAssignStmt( 0 , 6 , left, symbol, right) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 2 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*symbol {
                                    Token::PyMulAssign(2, 4, _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*right {
                                    ASTNode::AtomName( 5, 6, _ ) => assert!(true),
    ...
    #[test]
    fn statements_expression_modulo_assign() {
        let lexer = Box::new(PythonCoreTokenizer::new("a %= b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
            ...
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 9, nodes, separators, nwl) => {
                        ...
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::ModuloAssignStmt( 0 , 6 , left, symbol, right) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 2 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*symbol {
                                    Token::PyModuloAssign(2, 4, _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*right {
                                    ASTNode::AtomName( 5, 6, _ ) => assert!(true),
    ...
    #[test]
    fn statements_expression_matrice_assign() {
        let lexer = Box::new(PythonCoreTokenizer::new("a @= b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
            ...
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 9, nodes, separators, nwl) => {
                        ...
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::MatriceAssignStmt( 0 , 6 , left, symbol, right) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 2 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*symbol {
                                    Token::PyMatriceAssign(2, 4, _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*right {
                                    ASTNode::AtomName( 5, 6, _ ) => assert!(true),
    ...
    #[test]
    fn statements_expression_bitwise_and_assign() {
        let lexer = Box::new(PythonCoreTokenizer::new("a &= b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
            ...
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 9, nodes, separators, nwl) => {
                        ...
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::BitAndAssignStmt( 0 , 6 , left, symbol, right) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 2 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*symbol {
                                    Token::PyBitAndAssign(2, 4, _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*right {
                                    ASTNode::AtomName( 5, 6, _ ) => assert!(true),
    ...
    #[test]
    fn statements_expression_bitwise_or_assign() {
        let lexer = Box::new(PythonCoreTokenizer::new("a |= b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
            ...
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 9, nodes, separators, nwl) => {
                        ...
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::BitOrAssignStmt( 0 , 6 , left, symbol, right) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 2 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*symbol {
                                    Token::PyBitOrAssign(2, 4, _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*right {
                                    ASTNode::AtomName( 5, 6, _ ) => assert!(true),
    ...
    #[test]
    fn statements_expression_bitwise_xor_assign() {
        let lexer = Box::new(PythonCoreTokenizer::new("a ^= b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
            ...
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 9, nodes, separators, nwl) => {
                        ...
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::BitXorAssignStmt( 0 , 6 , left, symbol, right) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 2 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*symbol {
                                    Token::PyBitXorAssign(2, 4, _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*right {
                                    ASTNode::AtomName( 5, 6, _ ) => assert!(true),
    ...
    #[test]
    fn statements_expression_bitwise_shift_left_assign() {
        let lexer = Box::new(PythonCoreTokenizer::new("a <<= b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
        match &res {
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 10, nodes, separators, nwl) => {
                        assert_eq!(nodes.len(), 1);
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::ShiftLeftAssignStmt( 0 , 7 , left, symbol, right) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 2 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*symbol {
                                    Token::PyShiftLeftAssign(2, 5, _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*right {
    ...
    #[test]
    fn statements_expression_bitwise_shift_right_assign() {
        let lexer = Box::new(PythonCoreTokenizer::new("a >>= b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
            ...
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 10, nodes, separators, nwl) => {
                        ...
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::ShiftRightAssignStmt( 0 , 7 , left, symbol, right) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 2 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*symbol {
                                    Token::PyShiftRightAssign(2, 5, _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*right {
                                    ASTNode::AtomName( 6, 7, _ ) => assert!(true),
    ...
    #[test]
    fn statements_expression_bitwise_power_assign() {
        let lexer = Box::new(PythonCoreTokenizer::new("a **= b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
            ...
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 10, nodes, separators, nwl) => {
                        ...
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::PowerAssignStmt( 0 , 7 , left, symbol, right) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 2 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*symbol {
                                    Token::PyPowerAssign(2, 5, _ ) => assert!(true),
                                    _ => assert!(false)
                                }
                                match &*right {
                                    ASTNode::AtomName( 6, 7, _ ) => assert!(true),
    ...
    #[test]
    fn statements_expression_bitwise_floor_div_assign() {
        let lexer = Box::new(PythonCoreTokenizer::new("a //= b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
            ...
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 10, nodes, separators, nwl) => {
                        ...
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::FloorDivAssignStmt( 0 , 7 , left, symbol, right) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 2 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
    ...
    #[test]
    fn statements_expression_bitwise_floor_div_assign_yield() {
        let lexer = Box::new(PythonCoreTokenizer::new("a //= yield b\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
        match &res {
                    ...
                    ASTNode::SimpleStmtList(0, 16, nodes, separators, nwl) => {
                        ...
                        match node {
                            ASTNode::FloorDivAssignStmt( 0 , 13 , left, symbol, right) =>  {
                                ...
                                match &*right {
                                    ASTNode::YieldExpr( 6, 13, _, _) => assert!(true),
                                    _ => assert!(false)
                                }
                            },
                            _ => assert!(false)
    ...
    #[test]
    fn statements_expression_single_assign_testlist() {
        let lexer = Box::new(PythonCoreTokenizer::new("a = a, b, c\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
            ...
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 14, nodes, separators, nwl) => {
                        ...
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::AssignmentStmt( 0 , 11 , left, right_nodes, tc) =>  {
    ...
    #[test]
    fn statements_expression_single_assign_with_annotation_and_assignment() {
        let lexer = Box::new(PythonCoreTokenizer::new("a: int = v\r\n".to_string()));
        let mut parser = PythonCoreParser::new(lexer);
        parser.advance();
        let res = parser.parse_statements_stmt();
        match &res {
            Ok(s) => {
                match &**s {
                    ASTNode::SimpleStmtList(0, 13, nodes, separators, nwl) => {
                        assert_eq!(nodes.len(), 1);
                        let node = (*nodes[0]).clone();
                        match node {
                            ASTNode::AnnAssignStmt(0, 10, left, symbol, right, next) =>  {
                                match &*left {
                                    ASTNode::AtomName( 0, 1 , _ ) => assert!(true),
                                    _ => assert!(false)
                                }
